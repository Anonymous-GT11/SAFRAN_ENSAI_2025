...
# In[304]:
def unique_solutions(solutions, tol=1e-5):
        uniq = []
        for x, fit in solutions:
            if not any(np.allclose(x, u, atol=tol) for u, _ in uniq):
                uniq.append((x, fit))
        return uniq
...
class GeneticAlgorithm:
    ...
    def export_multiple_solutions_df(self, row_index: int, threshold_factor : float=2) -> pd.DataFrame:
        ...
        # To avoid many duplicated in the final data
    ...
    for data_type, df in [("clean", df_clean), ("noisy", df_noisy)]:
        ...
        for contexts in CONTEXT_SCENARIOS:
            ...
            for sensors in SENSOR_SCENARIOS:
                ...
                #--------------------------
                dfmultiple_solutions.to_csv ( os.path.join (scenario_dir, "multiple_solutions.csv") , index=False)
                #--------------------------
